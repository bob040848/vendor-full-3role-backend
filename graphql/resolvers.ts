...
export const resolvers: GQL_Resolvers = {
  ...
  Query: {
    ...
    salesSummary: async (
      _parent,
      { shop_id, vendor_id, start_date, end_date },
      { prisma }
    ) => {
      const where: any = {};
      if (shop_id) where.shop_id = shop_id;
      ...
      const summary = reports.reduce(
        (acc: SalesSummary, report: DailySalesReport) => ({
          ...
          pending_payments:
            acc.pending_payments + (report.pending_payments || 0),
        }),
        {
          total_sales: 0,
          total_orders: 0,
      ...
      const vendor = vendor_id
        ? await prisma.vendor.findUnique({ where: { id: vendor_id } })
        : null;
  ...
  Mutation: {
    ...
    generateDailySalesReport: async (
      _parent,
      { shop_id, vendor_id, date },
      context
    ) => {
      requireRole(context, ["VENDOR", "ADMIN"]);
